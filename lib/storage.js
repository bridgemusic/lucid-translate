...
const DEFAULTS = {
  ...
  // 自动检测：打开匹配源语言的页面时，右上角弹窗提示可翻译。默认开启。
  autoDetect: true,
  // 自动弹窗黑名单（域名，命中则不再自动提示）。仅影响弹窗，不影响手动翻译。
  blocklist: [],
};

// —— 纯函数：域名归一与匹配（无 chrome 依赖，便于测试）——
...
export function hostInList(host, list) {
  ...
  return list.some((item) => normalizeHostname(item) === h);
}
...
export async function addToBlocklist(hostname) {
  ...
  if (!hostInList(h, blocklist)) {
    await chrome.storage.local.set({ blocklist: [...blocklist, h] });
  }
}

// 查询域名是否在自动弹窗黑名单中。
export async function isBlocklisted(hostname) {
  const { blocklist = [] } = await chrome.storage.local.get("blocklist");
  return hostInList(hostname, blocklist);
}

// 清空自动弹窗黑名单（设置页"恢复所有网站提示"用）。

...
(function () {
  ...
  // 替换型 / 行内级元素：永远不应被当作「块级子元素」。它们由内联标签保护机制
  // 以占位符处理（见 inline-tags.js 的 REPLACE_TAGS）。单列出来是因为浏览器对
  // <img> 等的 computed display 不一定是 inline，且 jsdom 不提供 UA 样式时为空串，
  // 若走 isBlockLike 的 display 兜底会被误判为块级，导致父块被当成非叶子而漏翻。
  const REPLACED_TAGS = new Set([
    "IMG", "SVG", "VIDEO", "AUDIO", "CANVAS", "PICTURE", "OBJECT", "EMBED", "IFRAME", "MATH",
  ]);
    ...
    for (const child of el.childNodes) {
      if (child.nodeType === Node.TEXT_NODE) {
        ...
        if (REPLACED_TAGS.has(tag)) {
          // 替换型元素（img/svg/video…）不算块级子元素，也不贡献文本。
          // 它们由占位符机制处理；不在此把父块判成非叶子（否则图文段落漏翻）。
          continue;
        }
  ...
  function containsBlock(el) {
    for (const child of el.children) {
      ...
      if (REPLACED_TAGS.has(child.tagName)) continue; // 替换型元素不算块级
